...
then 
  ...
  _message " > @BLU@Removing previous catalogue e-g catalogue for @RED@${current##*/}@DEF@"
  rm -f ${outputname}
  _message " - @RED@Done! (`date +'%a %H:%M'`)@DEF@\n"
fi 
#Construct the output tomographic bin 
_message " > @BLU@Constructing g's from e's in catalogue for @RED@${current##*/}@DEF@"
@PYTHON3BIN@ @RUNROOT@/@SCRIPTPATH@/convert_e_to_g.py \
  --inpath ${current} \
  --outpath ${outputname} \
